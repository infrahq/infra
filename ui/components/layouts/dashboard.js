      ...
      <nav className='flex-none flex w-64 lg:w-72 flex-col inset-y-0 px-2 overflow-y-auto'>
        ...
        <div className='relative group mx-2 my-5 h-16 hover:h-[178px] hover:bg-purple-100/5 transition-height transition-size px-4 duration-300 ease-in-out rounded-xl overflow-hidden'>
          <div className='flex items-center space-x-4 mt-4 mb-2'>
            ...
            <div>
              <div className='text-gray-300 text-sm overflow-hidden overflow-ellipsis leading-none'>{auth?.name}</div>
              {admin && <div className='text-gray-300 text-xs leading-none my-1 capitalize'>Admin</div>}
            </div>
          </div>
          ...
          <div className='px-2 pt-1 pb-3 text-xs text-gray-300'>
            Infra version {version?.version}
          </div>
        </div>
      </nav>
      <main className='w-full overflow-x-hidden overflow-y-scroll'>
        <div className='mx-auto xl:max-w-4xl 2xl:max-w-5xl'>
          {children}
        </div>
      </main>
    </div>
