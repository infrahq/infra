      ...
      <div className='flex flex-col mt-6 mx-4 space-y-1'>
        <div className='mt-4'>
          ...
          <input
            ...
            className={`w-full bg-transparent border-b border-gray-950 text-label px-px py-3 focus:outline-none focus:border-b focus:border-gray-200 placeholder:italic ${error ? 'border-pink-500' : 'border-gray-800'}`} />
        </div>
...
function UserOneTimePassword ({password, handleAddUserOnClick}) {
  return (
    <div className='flex flex-col pt-8 px-1 border rounded-lg border-gray-950'>
      ...
      <h2 className='text-name pt-5 px-4'>User added. Send the user this one time password for their initial login. This password will not be shown again.</h2>
      <div className='flex flex-col mt-6 mx-4 space-y-1'>
        <label className='text-label text-gray-300 uppercase'>One Time Password</label>
        <input
          readOnly
          value={password}
          className='w-full bg-transparent text-label px-px py-3 focus:outline-none font-mono' />
      </div>
      <div className='flex flex-row justify-between m-6 items-center'>
        <button onClick={handleAddUserOnClick} className='uppercase border-0 hover:text-white text-gray-300 text-secondary'>Add Another</button>
        <Link href='/users'>
