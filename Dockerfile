...
RUN CGO_ENABLED=1 GOOS=linux GOARCH=$TARGETARCH CC=$TARGETARCH-linux-gnu-gcc go build -ldflags '-s -X github.com/infrahq/infra/internal.Version='"$BUILDVERSION"' -linkmode external -w -extldflags "-static"' .
