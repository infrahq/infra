...
CREATE TABLE `identities` (`id` integer,`created_at` datetime,`updated_at` datetime,`deleted_at` datetime,`kind` text,`name` text,`last_seen_at` datetime,`created_by` integer,PRIMARY KEY (`id`));
CREATE TABLE `groups` (`id` integer,`created_at` datetime,`updated_at` datetime,`deleted_at` datetime,`name` text,`created_by` integer,PRIMARY KEY (`id`));
...
CREATE TABLE `settings` (`id` integer,`created_at` datetime,`updated_at` datetime,`deleted_at` datetime,`private_jwk` blob,`public_jwk` blob,`signup_enabled` numeric,PRIMARY KEY (`id`));
...
CREATE TABLE `provider_users` (`id` integer,`created_at` datetime,`updated_at` datetime,`deleted_at` datetime,`provider_id` integer,`identity_id` integer,`email` text,`groups` text,`last_update` datetime,`redirect_url` text,`access_token` text,`refresh_token` text,`expires_at` datetime,PRIMARY KEY (`id`));
CREATE UNIQUE INDEX `idx_identities_name` ON `identities`(`name`) WHERE deleted_at is NULL;
CREATE UNIQUE INDEX `idx_groups_name_provider_id` ON `groups`(`name`) WHERE deleted_at is NULL;
