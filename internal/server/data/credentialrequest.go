...
func GetCredentialRequest(tx ReadTxn, id, organizationID uid.ID) (*models.CredentialRequest, error) {
	...
	err := tx.QueryRow(query.String(), query.Args...).Scan(r.ID, r.OrganizationID, r.ExpiresAt, r.UpdateIndex, r.UserID, r.DestinationID)
	if err != nil {
		return nil, err
	}
	return r, nil
...
func CreateCredentialRequest(tx WriteTxn, cr *models.CredentialRequest) error {
	...
	_, err := tx.Exec(q.String(), q.Args...) // will trigger credential_request_update_notify()
	if err != nil {
...
func UpdateCredentialRequest(tx WriteTxn, cr *models.CredentialRequest) error {
	...
	q.B("SET")
	q.B("answered = ?,", true)
	q.B("bearer_token = ?,", cr.BearerToken) // TODO(steve): set fields
	q.B("WHERE id = ?", cr.ID)
...
func ListCredentialRequests(tx ReadTxn, destinationID uid.ID) ([]models.CredentialRequest, error) {
	...
	return scanRows(rows, func(credentialReq *models.CredentialRequest) []any {
		return []any{"id", "organization_id", "expires_at", "update_index", "user_id", "destination_id"}
	})
}
