...
func (a *oidcAuthn) Authenticate(ctx context.Context, db *gorm.DB) (*models.Organization, *models.Identity, *models.Provider, AuthScope, error) {
	...
	if err != nil {
		return nil, nil, nil, AuthScope{}, err
	}
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			return nil, nil, nil, AuthScope{}, fmt.Errorf("%w: %s", internal.ErrBadGateway, err.Error())
		}

		return nil, nil, nil, AuthScope{}, fmt.Errorf("exhange code for tokens: %w", err)
	}

	identity, err := data.GetIdentity(db.Preload("Groups"), data.ByName(email))
	if err != nil {
		if !errors.Is(err, internal.ErrNotFound) {
			return nil, nil, nil, AuthScope{}, fmt.Errorf("get user: %w", err)
		}

		identity = &models.Identity{Name: email}

		if err := data.CreateIdentity(db, identity); err != nil {
			return nil, nil, nil, AuthScope{}, fmt.Errorf("create user: %w", err)
		}
	}

	providerUser, err := data.CreateProviderUser(db, provider, identity)
	if err != nil {
		return nil, nil, nil, AuthScope{}, fmt.Errorf("add user for provider login: %w", err)
	}
	...
	if err != nil {
		return nil, nil, nil, AuthScope{}, fmt.Errorf("UpdateProviderUser: %w", err)
	}
	...
	if err != nil {
		return nil, nil, nil, AuthScope{}, fmt.Errorf("sync user on login: %w", err)
	}

	return org, identity, provider, AuthScope{}, nil
}
