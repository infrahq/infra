...
func TestAPI_ListGrants(t *testing.T) {
	...
	testCases := map[string]testCase{
		...
		"with stale update index": {
			urlPath: "/api/grants?resource=res1&lastUpdateIndex=1",
			setup: func(t *testing.T, req *http.Request) {
				req.Header.Set("Authorization", "Bearer "+adminAccessKey(srv))
			},
			expected: func(t *testing.T, resp *httptest.ResponseRecorder) {
				...
				expected := []api.Grant{
					...
					{User: idOther, Privilege: "custom2", Resource: "res1"},
					{User: idOther, Privilege: "connector", Resource: "res1"},
				}
				assert.DeepEqual(t, grants.Items, expected, cmpAPIGrantShallow)
		...
		"with latest update index and timeout": {
			urlPath: "/api/grants?resource=res1&lastUpdateIndex=100100",
			setup: func(t *testing.T, req *http.Request) {
				req.Header.Set("Authorization", "Bearer "+adminAccessKey(srv))
